...
1. Загрузка резюме `.pdf` (или вставка текста) → `parse_resume` строит профиль (навыки, роли, вилка ЗП)
2. AI предлагает настройки поиска (`suggest_filters`) — должности, ключи, регионы
3. Скан подтягивает вакансии с **hh.ru**
4. **Fit Score** (`domain/fit_score.py`) — взвешенные must/nice навыки, роль, опыт, домен, evidence
5. Сопроводительное + AI-рекомендации — черновики AI, отправляете вы сами

```mermaid
flowchart LR
    R[Резюме .pdf] --> P[Профиль кандидата]
...
| `RESUME_PATH` | Путь к резюме `.pdf` / `.md` / `.txt` |
